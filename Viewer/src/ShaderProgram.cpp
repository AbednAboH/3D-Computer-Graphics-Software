...
bool ShaderProgram::loadShaders(const char* vsFilename, const char* fsFilename)
{
	...
	glCompileShader(vs);
	checkCompileErrors(vs, VERTEX, vsFilename);

	glCompileShader(fs);
	checkCompileErrors(fs, FRAGMENT, fsFilename);
	...
	checkCompileErrors(programHandle, PROGRAM,fsFilename);
...
//-----------------------------------------------------------------------------
void  ShaderProgram::checkCompileErrors(GLuint shader, ShaderType type,const char * name)
{
	int status = 0;
	...
	if (type == PROGRAM)
	{
		...
		if (status == GL_FALSE)
		{
			...
			std::cerr << "Error! Shader program failed to link. "<<name<<" " << errorLog << std::endl;
		}
	}
	else
	{
